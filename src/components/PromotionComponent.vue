...
function show_alert(input: string){
...
<template>
  <div class="poster_list" role="list">
    <div v-for="item in promotion_state.get_promotions()" class="poster_item" role="listitem" :key="item.id" :style="{ backgroundColor: item.color }">
      <div class="poster_main">
        <span class="poster_label">{{ item.title }}</span>
        <ButtonComponent @click="show_alert(item.title)" :style="{ backgroundColor: item.buttonColor }"/>
      </div>
